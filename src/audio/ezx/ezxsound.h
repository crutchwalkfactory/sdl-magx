...
{
	SOUNDM_AUDIO_FLASH_CLIENT = 0x4080803,
};
...
enum AM_HW_DEV_UPDATED_STATE_ENUM_T
{
    AM_HW_DEV_UPDATED_STATE_disable,
    AM_HW_DEV_UPDATED_STATE_enable,	
};

enum AM_HW_DEV_CLIENT_CTL_CMD_ENUM_T
{
	AM_HW_DEV_CLIENT_CTL_CMD_vol_get,
	AM_HW_DEV_CLIENT_CTL_CMD_vol_set
};

typedef void (*AM_HW_DEV_CALLBACK_T)(AM_HW_DEV_UPDATED_STATE_ENUM_T, void*);

class AM_HW_DEV_INTERFACE
{
		char fix[24];
	...
	public:
		...
		void am_hwdev_verify_freq(unsigned int);
		void callbackfunc_init( AM_HW_DEV_CALLBACK_T, void*);
		void close();
		void dev_ioctl(AM_HW_DEV_CLIENT_CTL_CMD_ENUM_T, void*);
		void flush();
		void open();
		void open(unsigned int, unsigned int, unsigned int);
		void write(short*, unsigned int);
};
