...
void SDL_ChannelExists(const char *channelName)
{
  SDL_MotoChannel = "PE/SDLLib/";//channelName;
  printf("'%s' - channel, run application\n", channelName);
...
//  : QWidget(0, "SDL_main", WType_TopLevel | WStyle_Customize | WStyle_NoBorder),
      : ZKbMainWidget ( (ZHeader::HEADER_TYPE)3, 0, "SDL_main", WType_TopLevel | WStyle_Customize | WStyle_NoBorder),
		my_image(0),
    ...
    my_suspended(false), last_mod(false)
{
  ...
  connect(qApp, SIGNAL(signalRaise()), this, SLOT(signalRaise()));
  connect(qApp, SIGNAL(askReturnToIdle(int)), this, SLOT(signalAskReturnToIdle(int)));  
...
void SDL_QWin::channel(const QCString &msg, const QByteArray &)
{
...
void SDL_QWin::signalAskReturnToIdle(int)
{
	printf("signal askReturnToIdle\n");
	suspend();
}
...
/*
bool SDL_QWin::eventFilter(QObject* o, QEvent* pEvent)
{
    ...
    return ZKbMainWidget::eventFilter( o, pEvent ); 
}
*/
...
{  
  ...
  if(scancode >= 'A' && scancode <= 'Z') {
    ...
    switch(scancode) {
      case 0x1031: //Cancel
        scancode = SDLK_ESCAPE;//my_special ? SDLK_a : 
      break;
      case 0x1004: //Joystick center
        scancode = SDLK_RETURN;//my_special ? SDLK_b : 
      break;
      ...
      break;
      case 0x1005: //special key SIDE SELECT
        scancode =  SDLK_SPACE;//my_special ? SDLK_a :
      break;
      case 0x1033: //special key CARRIER 
        scancode = SDLK_PAUSE;
      break;      
	  //	if (pressed)
	  //		system("/mmc/mmca1/.Programs/zTaskMan/zTaskMan&");
	  //return;
      //scancode = SDLK_F10;
      //break; 
	  ...
	  // end functional keys
	  case 0x1016: //VolUp
        scancode = SDLK_PLUS;//my_special ? SDLK_c : 
      break;
      case 0x1017: //VolDown
        scancode = SDLK_MINUS;//my_special ? SDLK_d : 
      break;
