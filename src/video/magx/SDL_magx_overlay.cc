...
{
	printf("MAGX: Create overlay: width %d, height %d\n", width, height);
	...
	swdata->size = width*height*2;
	swdata->addr = ipu_malloc(swdata->size);
	if ( swdata->addr )
	{
		SDL_FreeYUVOverlay(overlay);
		return(NULL);
	}
	swdata->pixels = (Uint8 *)mmap (NULL, swdata->size, 
...
int SDL_LockYUV_HW(_THIS, SDL_Overlay *overlay)
{
	return(0);
}

void SDL_UnlockYUV_HW(_THIS, SDL_Overlay *overlay)
{
	return;
}

int SDL_DisplayYUV_HW(_THIS, SDL_Overlay *overlay, SDL_Rect *src, SDL_Rect *dst)
{
	struct private_yuvhwdata *swdata;
	int stretch;
	int scale_2x;
...
void SDL_FreeYUV_HW(_THIS, SDL_Overlay *overlay)
{
	printf("MAGX: Free overlay\n");
	...
	if ( swdata ) {
		...
		if ( swdata->pixels ) 
		{
			munmap(swdata->pixels, swdata->size);
			ipu_free(swdata->addr);
		}
		if ( swdata->colortab ) {
